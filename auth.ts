...
import {
  cfg, q, ok, AppError, zodHook, authBodySchema,
  type AppEnv,
} from "./types.ts";
import { db, run, getTable } from "./db.ts";
...
async function findUser(username: string) {
  const rows = await run(
    `SELECT id, username, password FROM ${q(cfg.authTable)} WHERE username = $1`,
    [username],
  );
...
export function registerAuthRoutes(app: Hono<AppEnv>) {
  ...
  app.post(
    ...
    async (c) => {
      ...
      await run(
        `INSERT INTO ${q(cfg.authTable)} (username, password) VALUES ($1, $2)`,
        [username, password],
      );
      const user = await findUser(username);
      return c.json(ok(await issueToken(user.id, username)));
  ...
  app.get("/api/auth/profile", async (c) => {
    ...
    const rows = await run(
      `SELECT * FROM ${q(cfg.authTable)} WHERE id = $1`,
      [userId],
    );
  ...
  app.post("/api/auth/profile", async (c) => {
    ...
    vals.push(userId);
    await run(
      `UPDATE ${q(cfg.authTable)} SET ${sets.join(", ")} WHERE id = $${n}`,
      vals,
    );
