...
import { cfg, q, ok, paged, ownerCond, AppError, type AppEnv } from "./types.ts";
import { db, run, getTable, isAuthTable, type TblMeta } from "./db.ts";
import {
...
export function registerCrudRoutes(app: Hono<AppEnv>) {
  ...
  app.post("/api/query/:table", async (c) => {
    const tbl = requireTable(c.req.param("table"));
    const userId = c.get("userId");
    const body = (await c.req.json()) as BodyQuery;
  ...
  app.post("/api/delete/:table", async (c) => {
    const tbl = requireTable(c.req.param("table"));
    const userId = c.get("userId");
    const body = (await c.req.json()) as BodyWhereInput;
  ...
  app.get("/api/data/:table/:id", async (c) => {
    ...
    let sql = `SELECT * FROM ${q(tName)} WHERE ${q(tbl.pk)} = $1`;
    ...
    for (const item of items) {
      if (tbl.pk && item[tbl.pk] !== undefined) {
        const exist = await run(
          `SELECT 1 FROM ${q(tbl.name)} WHERE ${q(tbl.pk)} = $1`,
          [item[tbl.pk]],
        );
        if (exist.length > 0)
          throw new AppError(
  ...
  app.delete("/api/data/:table/:id", async (c) => {
    ...
    let sql = `DELETE FROM ${q(tName)} WHERE ${q(tbl.pk)} = $1`;
    if (tbl.hasOwner) {
      sql += ` AND ${ownerCond(2)}`;
      params.push(c.get("userId"));
    }
...
async function collectDeletedIds(
  ...
  if (!tbl.pk) return [];
  /* 把 "DELETE FROM ..." 替换为 "SELECT pk FROM ..." */
...
async function existsRow(
  ...
  const params: unknown[] = [pkVal];
  let sql = `SELECT 1 FROM ${q(tbl.name)} WHERE ${q(tbl.pk!)} = $1`;
  if (tbl.hasOwner) {
    sql += ` AND ${ownerCond(2)}`;
    params.push(userId);
  }
...
async function insertRow(
  ...
  await run(
    `INSERT INTO ${q(tbl.name)} (${cols.map(q).join(", ")}) VALUES (${ph})`,
    vals,
  );
...
async function updateRow(
  ...
  let sql = `UPDATE ${q(tbl.name)} SET ${sets.join(", ")} WHERE ${q(tbl.pk!)} = $${n++}`;
