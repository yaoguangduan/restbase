 ...
 *   group: 字段名数组
 */
import type { TblMeta, ColMeta } from "./db.ts";
import { cfg, q, ownerCond, AppError } from "./types.ts";
...
interface Cond { sql: string; values: unknown[] }
...
export function buildDeleteSQL(
  ...
  const sql = `DELETE FROM ${q(tbl.name)} ${where}`;
...
/** select 项 */
export type BodySelectItem =
  | string                                       // "field" | "field:alias" | "func:field:alias"
  | { field: string; alias?: string; func?: string };

/** order 项 */
export type BodyOrderItem =
  | string                                       // "field" | "asc.field" | "desc.field"
  | { field: string; dir?: "asc" | "desc" };

/** 完整查询 Body */
export interface BodyQuery {
  select?: BodySelectItem[];
...
export function buildBodyDeleteSQL(
  ...
  return { sql: `DELETE FROM ${q(tbl.name)} ${w.where}`, values: [...w.values] };
}
